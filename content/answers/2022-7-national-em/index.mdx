...
전하량 $q$을 갖고 있는 전하의 모임들이 여러 개가 있는데, 이 모임들이 이동하는 속력이 $1$이고, 이 모임들은 서로 $1\cdot1=v$(속력 단위시간과 값만 같을 뿐 완전히 다른 물리량이다.
속력 $1$(차원은 거리/시간)에 단위시간 $1$(차원은 시간)이 곱해진 $1$ 거리의 물리량이기 때문이다.)의 간격을 두고 있다고 하자.
...
$$
이다.
이제 세 번째 양전하가 무한히 먼 곳에서 남은 한 자리로 배치되는 것을 생각해보자. 첫 번째 양전하로 인한 정전에너지는 거리와 전하량이 동일하므로 두 번째 양전하가 이동해올 때의 정전에너지와 동일하다.
다음으로 두 번째 정전하로 인한 정전에너지도 거리와 전하량이 동일하므로 첫 번째 양전하로 인한 정전에너지와 같다. 따라서 총 정전에너지는
$$
W=3\sqrt{3}+3\sqrt{3}+3\sqrt{3}=9\sqrt{3}\text{ nJ}
$$
이므로 답은 **3번**이다.
...
## 14번
경계면에서 자유전하는 없으므로 경계면의 수직 방향으로 $\vec{D}$의 변화는 없다. 만약 변화가 있다면 그 차이를 만들어내는 전하가 있을 것이기 때문이다.
가령 전기력선이 $3$만큼 들어가서 $3$이 나오면 아무 전하도 없는 것이지만, $2$가 들어가서 $4$가 나왔다면 양쪽으로 $1$만큼의 전기력선이 나가고 있는 것 아니겠는가.
...
\end{gather}
$$
다음으로, $\vec{E_2}$에는 시간 변수가 없으므로 정전자계임을 알 수 있고, 따라서 전기장의 회전은 없다. 이 말인 즉슨 경계면에 평행한 방향의 전기장 성분이 양쪽에서 똑같다는 것이다. 똑같지 않다면 그 차이를 일으키는 회전햐는 전기장이 있단 소리일 테니까. 
따라서 영역 $1$에서 경계면에 평행한 전기장 성분은 허무하게도
$$
\begin{gather}
...
&=-\frac{\partial}{\partial t}\left(aB_0\frac{1}{-\beta}\left(sin\left(\omega t-\frac{\beta b}{2}\right)-sin\left(\omega t+\frac{\beta b}{2}\right) \right)\right)\\
&=\frac{a B_0 \omega \left(cos(\omega t - \frac{\beta b}{2})-cos(\omega t + \frac{\beta b}{2}) \right)}{\beta}\\
&=B_0a\omega \frac{2sin(\omega t)sin\left(\frac{\beta b}{2}\right)}{\beta}\\
\end{split}
\end{equation}
$$
이므로 최댓값은
$$
emf_{max}=\frac{2B_0a\omega}{\beta}sin\left(\frac{\beta b}{2}\right)
$$
이므로 답은 **3번**이다.
